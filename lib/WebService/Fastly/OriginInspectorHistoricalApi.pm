...
#
package WebService::Fastly::OriginInspectorHistoricalApi;
...
# @param string $metric The metric to retrieve. Up to ten comma-separated metrics are accepted. (optional, default to 'responses')
...
# @param string $host Limit query to one or more specific origin hosts. Values should be comma-separated. (optional)
{
    ...
    'group_by' => {
        data_type => 'string',
        description => 'Dimensions to return in the query. Multiple dimensions may be separated by commas. For example, &#x60;group_by&#x3D;host&#x60; will return one timeseries for every origin host, as a total across all POPs. ',
        required => '0',
    },
    'limit' => {
        data_type => 'string',
    ...
    __PACKAGE__->method_documentation->{ 'get_origin_inspector_historical' } = {
        summary => 'Get historical origin data for a service',
        params => $params,
        returns => 'HistoricalOriginsResponse',
        };
}
# @return HistoricalOriginsResponse
#
sub get_origin_inspector_historical {
    my ($self, %args) = @_;

    # verify the required parameter 'service_id' is set
    unless (exists $args{'service_id'}) {
      croak("Missing the required parameter 'service_id' when calling get_origin_inspector_historical");
    }

    # parse inputs
    my $_resource_path = '/metrics/origins/services/{service_id}';
    ...
    my $_response_object = $self->{api_client}->deserialize('HistoricalOriginsResponse', $response);
