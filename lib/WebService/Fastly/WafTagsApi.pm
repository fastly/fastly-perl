...
#
package WebService::Fastly::WafTagsApi;
...
# @param string $include Include relationships. Optional. (optional, default to 'waf_rules')
{
    ...
    __PACKAGE__->method_documentation->{ 'list_waf_tags' } = {
        summary => 'List tags',
        params => $params,
        returns => 'WafTagsResponse',
        };
}
...
sub list_waf_tags {
    ...
    my $_response_object = $self->{api_client}->deserialize('WafTagsResponse', $response);
