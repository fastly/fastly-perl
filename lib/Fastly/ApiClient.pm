...
sub new {
    ...
    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'Fastly::Configuration' ) {
        $config = $_[0];
    } else {
        $config = Fastly::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("Fastly::Object::$class")->new;
        if (ref $data eq "HASH") {
