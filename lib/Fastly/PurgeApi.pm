...
# @param int $fastly_soft_purge Optional header indicating that the operation should be a &#39;soft&#39; purge, which marks the affected object as stale rather than making it inaccessible. (optional, default to 1)
{
    ...
    'host' => {
        data_type => 'string',
        description => 'The hostname for the content you&#39;ll be purging.',
        required => '1',
    },
    'fastly_soft_purge' => {
        data_type => 'int',
...
sub purge_single_url {
    ...
    unless (exists $args{'host'}) {
      croak("Missing the required parameter 'host' when calling purge_single_url");
    }
    ...
    # header params
    if ( exists $args{'host'}) {
        $header_params->{'host'} = $self->{api_client}->to_header_value($args{'host'});
    }
